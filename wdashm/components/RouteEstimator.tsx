...
import { Bike, Car, Clock, Footprints } from 'lucide-react';
    ...
    <div className="flex items-center gap-2 bg-slate-50 dark:bg-gray-800 border border-slate-100 dark:border-gray-700 p-2.5 rounded-xl mb-4 text-xs transition-colors">
      ...
      <div className="flex-1 flex justify-between items-center">
        ...
        <span className="text-gray-500 dark:text-gray-400 flex items-center gap-1">
          <Clock size={12} className="shrink-0" /> ~<strong className="text-brand-600 dark:text-brand-400">{displayMinutes} min {isReal && <span className="text-[10px] text-brand-600 font-bold">(itinéraire)</span>}</strong> en {vehicleType || 'moto'}
