                  ...
                  onClick={() => {
                    ...
                    if (notif.type === "verification_request") {
                      ...
                      if (order) {
                        ...
                        const orderIdStr = notif.data.order_id;
                        let extractedUserId = "";
                        if (orderIdStr.length >= 40) {
                          extractedUserId = orderIdStr.substring(4, 40);
                        } else {
                          const parts = orderIdStr.replace("sub-", "").split("-");
                          extractedUserId = parts.length >= 5 ? parts.slice(0, 5).join("-") : (parts[0] || "");
                        }
      ...
      .on(
        ...
        (payload) => {
          ...
          if (
            ...
            // Play sound if possible (only when notification is actually relevant)
            try {
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Décrivez votre offre (ex: -20% sur tout le menu)..."
                      value={newPromoCaption}
                      onChange={(e) => setNewPromoCaption(e.target.value)}
                    />
                  </div>

                  {/* LIEN PRODUIT PROMOTIONNEL */}
                  <div className="bg-purple-950/20 dark:bg-purple-900/10 border border-purple-500/20 rounded-[28px] p-6 space-y-4">
                    <h5 className="font-display font-black text-xs text-purple-600 dark:text-purple-400 uppercase tracking-widest flex items-center">
                      <Zap size={14} className="mr-2 text-yellow-500 animate-pulse animate-duration-1000" />
                      Lier à un produit du menu (Optionnel)
                    </h5>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <select
                          className="w-full px-4 py-3 glass border border-white/20 rounded-xl focus:ring-2 focus:ring-purple-500 outline-none dark:text-white bg-gray-900 font-medium"
                          value={promoLinkedItemId}
                          onChange={(e) => {
                            setPromoLinkedItemId(e.target.value);
                            if (e.target.value) {
                              const selectedItem = restaurant.menu.find(item => item.id === e.target.value);
                              if (selectedItem) {
                                const recommendedPrice = (selectedItem.price * 0.85).toFixed(2);
                                setPromoDiscountPrice(recommendedPrice);
                                setPromoBadgeText("-15%");
                                if (!newPromoCaption) {
                                  setNewPromoCaption(`Promotion spéciale sur notre ${selectedItem.name} !`);
                                }
                              }
                            } else {
                              setPromoDiscountPrice("");
                        ...
                        >
                          <option value="" className="text-gray-950 dark:text-white bg-gray-900">Aucun produit lié</option>
                          {restaurant.menu?.map(item => (
                            <option key={item.id} value={item.id} className="text-gray-950 dark:text-white bg-gray-900">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            {promotions.map((promo) => {
              const parsed = (() => {
                ...
                if (trimmed.startsWith('{') && trimmed.endsWith('}')) {
                  try {
                    ...
                    if (js && js.is_promo_product) {
                      return {
                        caption: js.caption || "Offre exceptionnelle !",
                        isPromo: true,
                        badge: js.badge_text,
              ...
              const linkedItem = parsed.isPromo && parsed.menuItemId
                ? restaurant.menu?.find(m => m.id === parsed.menuItemId)
                : null;
              ...
              return (
                ...
                >
                  {promo.mediaType === "video" ? (
                    ...
                    <img
                      src={linkedItem ? linkedItem.image : promo.mediaUrl}
                      alt="Promo"
                      className="w-full h-full object-cover opacity-70 group-hover/item:opacity-90 transition-opacity duration-700"
                      referrerPolicy="no-referrer"
                    />
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black via-black/45 to-transparent flex flex-col justify-end p-6 translate-y-2 group-hover/item:translate-y-0 transition-transform duration-500">
                    {parsed.isPromo && (
                      <div className="flex items-center space-x-1.5 mb-2">
                        <span className="text-[8px] font-black bg-purple-600 text-white px-2 py-0.5 rounded-md uppercase tracking-wider">
                          {parsed.badge || "PROMO"}
                        </span>
                        {linkedItem && (
                          <span className="text-[8px] font-black bg-white/20 text-white px-1.5 py-0.5 rounded-md uppercase tracking-widest truncate max-w-[100px]">
                            {linkedItem.name}
                          </span>
                        )}
                      </div>
                    )}
                    <p className="text-white font-display font-black text-sm uppercase tracking-tight mb-3 leading-snug drop-shadow-md">
                      {parsed.caption}
                    </p>
                    <div className="flex items-center justify-between">
